...
interface EventDetailProps {
  ...
  // optional override to force donor or receiver UI when rendered under a role-scoped route
  roleOverride?: "donor" | "receiver" | null;
}

export default function EventDetail({ params, roleOverride }: EventDetailProps) {
  ...
  // Resolve effective role: prefer explicit override (from donor/receiver wrapper), else use detected role
          ...
          <Card>
            <div className="p-4">
              ...
              <div className="mt-6 flex gap-2">
                {/* RSVP is available to everyone; Request to Collaborate only for donors */}
                {effectiveRole === "donor" && (
                  <SecondaryButton onClick={handleRequestCollaborate} disabled={hasRequested}>{hasRequested ? 'Requested - Pending' : 'Request to Collaborate'}</SecondaryButton>
                )}
                <OutlineButton onClick={handleChat}>Open Chat</OutlineButton>
              </div>
            </div>
          </Card>
        </div>
